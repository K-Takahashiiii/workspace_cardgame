...
<body>
	...
	<%
		...
		if (description == null)
			description = "テスト用の大会概要です。";

		String status = (String) request.getAttribute("status");
		if (status == null)
			status = "1"; // 1=受付中 をデフォに
		...
		if (registrationDeadline == null)
			registrationDeadline = "2026-01-31T23:59";
		...
		// 参加条件コード（初期値は0）
		String entryRequirement = (String) request.getAttribute("entryRequirement");
	...
	<form action="<%=request.getContextPath()%>/registerTournament"
		method="post">

		<p>
			大会名：<br> <input type="text" name="name" size="40" required
				value="<%=name%>">
		</p>

		<p>
			開催日：<br> <input type="date" name="eventDate" required
				value="<%=eventDate%>">
		</p>

		<p>
			開催時刻：<br> <input type="time" name="eventTime"
				value="<%=eventTime%>">
		</p>

		<p>
			最大参加人数：<br> <input type="number" name="maxParticipants" min="1"
				required value="<%=maxParticipants%>">
		</p>
		...
		<p>
			会場：<br> <input type="text" name="venue" size="40"
				value="<%=venue%>">
		</p>

		<p>
			使用ゲーム：<br> <input type="text" name="gameTitle" size="40"
				value="<%=gameTitle%>">
		</p>

		<p>
			形式：<br> <input type="text" name="tournamentFormat" size="40"
				value="<%=tournamentFormat%>">
		</p>

		<p>
			参加費(円)：<br> <input type="number" name="entryFeeYen" min="0"
				step="1" value="<%=entryFeeYen%>">
		</p>

		<p>
			登録締め切り：<br> <input type="datetime-local"
				name="registrationDeadline" value="<%=registrationDeadline%>">
		</p>

		<p>
			試合形式：<br> <input type="text" name="matchFormat" size="40"
				value="<%=matchFormat%>">
		</p>

		<p>
			制限時間(分)：<br> <input type="number" name="timeLimitMinutes"
				min="0" step="1" value="<%=timeLimitMinutes%>">
		</p>

		<p>
			引き分け時：<br> <input type="text" name="drawRule" size="40"
				value="<%=drawRule%>">
		</p>

		<p>
			優勝賞品：<br> <input type="text" name="prizeFirst" size="40"
				value="<%=prizeFirst%>">
		</p>

		<p>
			準優勝賞品：<br> <input type="text" name="prizeSecond" size="40"
				value="<%=prizeSecond%>">
		</p>

		<p>
			3位賞品：<br> <input type="text" name="prizeThird" size="40"
				value="<%=prizeThird%>">
		</p>
		...
		<h2>参加条件</h2>
		<p>
			参加条件コード：<br> <input type="number" name="entryRequirement"
				min="0" step="1" value="<%=entryRequirement%>"> <br>
			<small> 0：参加条件なし<br> 1：いままで３位以上になったことがある<br>
				2：対戦での総勝利数１０回以上<br> 3：対戦での総勝利数３回以上
			</small>
		</p>
